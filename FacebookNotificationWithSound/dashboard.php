...
if(isset($_POST['submit'])){
	...
	mysqli_query($con,"insert into fb_messages(from_id,to_id,message) values('$uid','$to_id','$message')");
	$msg="Message send";
}
//join query
$res_message=mysqli_query($con,"select fb_user.name,fb_messages.message from fb_messages,fb_user where fb_messages.status=0 and fb_messages.to_id='$uid' and fb_user.id=fb_messages.from_id");
$unread_count=mysqli_num_rows($res_message);
//user show
$sql_user="select id,name from fb_user order by name asc";
